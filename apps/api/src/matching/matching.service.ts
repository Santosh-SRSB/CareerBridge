...
export class MatchingService {
  ...
  async recomputeMatchesForJob(jobId: string) {
    ...
    const jobVector = await this.aiGateway.getEmbeddingVector('JOB', job.id);
    ...
    const candidates = await this.prisma.candidate.findMany({
      include: {
        ...
        resumes: { select: { id: true, rawText: true, summary: true }, take: 1 },
      },
      take: 200,
    });
    ...
    for (const candidate of candidates) {
      ...
      const totalScore =
        semanticScore > 0
          ? Math.min(100, Math.round(ruleScore * 0.7 + semanticScore * 0.3))
          : ruleScore;

      const reasons = [...base.reasons];
      if (semanticScore >= 70) reasons.push('Strong profile match');
      else if (semanticScore >= 50) reasons.push('Related experience');
    ...
    // Optional Gemini enrichment for top candidates only (cost control).
    if (this.config.get<string>('AI_MATCH_ENRICH') === 'true' && this.aiGateway.isConfigured()) {
      scored.sort((a, b) => b.totalScore - a.totalScore);
      const enrichTargets = scored.slice(0, 5);
      for (const item of enrichTargets) {
        const candidate = candidates.find((c) => c.id === item.candidateId);
        if (!candidate) continue;
        const ai = await this.aiGateway.matchJob(
          {
            city: candidate.city,
            skills: candidate.skills.map((s) => s.name),
            careerInterests: parseList(candidate.careerInterests),
        ...
        if (ai?.matchScore != null) {
          item.totalScore = Math.min(
            100,
            Math.round(item.totalScore * 0.8 + Number(ai.matchScore) * 0.2),
          );
          if (ai.matchedSkills?.length) {
            item.reasons = [...new Set([...item.reasons, ...ai.matchedSkills.slice(0, 3)])];
          }
          if (ai.missingSkills?.length) {
            item.gaps = [...new Set([...item.gaps, ...ai.missingSkills.slice(0, 5)])];
          }
        }
      }
    }
  ...
  /** Refresh employer match pools after a candidate finishes onboarding / profile changes. */
  async recomputeMatchesForPublishedJobs() {
    ...
    for (const job of jobs) {
      await this.recomputeMatchesForJob(job.id);
    }
  }
