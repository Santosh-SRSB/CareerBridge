...
export class JobsService {
  ...
  async list(
    ...
    return {
      items: rows.map((job) => this.toCard(job, candidate)),
      page,
      pageSize,
      total,
    };
  ...
  async recommended(userId: string) {
    const result = await this.list({ page: 1, pageSize: 10 }, userId);
    return {
      ...result,
      items: [...result.items].sort((a, b) => (b.match?.score || 0) - (a.match?.score || 0)).slice(0, 6),
    };
  }
