...
export class HumanMocksService {
  ...
  async schedule(
    userId: string,
    input: { candidateName: string; candidateEmail: string; scheduledAt: string },
  ) {
    ...
    const interviewType = humanInterviewTypeFromRole(jobRole);
    ...
    const created = await this.prisma.humanMockInterview.create({
      data: {
        ...
        interviewerName: HUMAN_INTERVIEWER.name,
        interviewerEmail: HUMAN_INTERVIEWER.email,
        candidateName,
        candidateEmail,
    ...
    return { ...this.toSession({ ...created, emailSentAt: emailSent ? new Date() : null }), emailSent };
  }
