...
import { getCandidateMe, getProfileCompletion, createResume, downloadResume, saveBase64File } from '@/lib/api';
...
async function saveAndDownloadResume(targetJobTitle?: string) {
  const resume = await createResume({
    targetJobTitle: targetJobTitle || undefined,
    template: 'CLASSIC',
  });
  const file = await downloadResume(resume.id);
        ...
        <footer className="cb-profile-footer">
          ...
          <Button
            ...
            onClick={() => {
              setSaveError('');
              setSaving(true);
              saveAndDownloadResume(profile.careerInterests[0])
                .then(() => {
