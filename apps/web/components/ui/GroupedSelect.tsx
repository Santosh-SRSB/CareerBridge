...
export function GroupedSelect({
  ...
  return (
    <div ref={rootRef} className="relative min-w-0 space-y-2">
      ...
      {open ? (
        <div className="absolute z-50 mt-1 w-full overflow-hidden rounded-md border border-primary/15 bg-white shadow-[0_16px_40px_rgba(12,51,64,0.16)]">
          <div className="max-h-56 overflow-y-auto" role="listbox">
            {allowAll ? (
              <button
                type="button"
                className={`flex w-full px-3 py-2 text-left text-sm ${!value && !otherMode ? 'bg-[#e8fbfa] font-bold text-primary' : 'text-primary hover:bg-[#f7fbfb]'}`}
                onClick={() => pick('')}
              >
                {allLabel}
              </button>
            ...
            {groups.map((group) => (
              <div key={group.label || 'options'}>
                {group.label ? (
                  <p className="sticky top-0 bg-[#f3f7f7] px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[0.14em] text-muted">
                    {group.label}
                  </p>
                ) : null}
                {group.options.map((option) => (
                  <button
                    key={option}
                    type="button"
                    className={`flex w-full px-3 py-2 text-left text-sm ${!otherMode && value === option ? 'bg-[#e8fbfa] font-bold text-primary' : 'text-primary hover:bg-[#f7fbfb]'}`}
                    onClick={() => pick(option)}
                  >
                    {option}
                  </button>
          ...
          <button
            type="button"
            className={`flex w-full border-t border-primary/10 px-3 py-2.5 text-left text-sm font-bold ${otherMode ? 'bg-[#e8fbfa] text-primary' : 'text-primary hover:bg-[#f7fbfb]'}`}
            onClick={() => pick('', true)}
          >
            Others
          </button>
