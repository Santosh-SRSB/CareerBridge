...
function PassportCrest({ light = false }: { light?: boolean }) {
  const mark = light ? '#0c3340' : '#fff';
  return (
    <svg viewBox="0 0 40 40" className="h-9 w-9 shrink-0" aria-hidden>
      <circle cx="20" cy="20" r="18" fill="none" stroke="#e8c56b" strokeWidth="1.4" />
      <circle cx="20" cy="20" r="14" fill="none" stroke="#1ec8c0" strokeWidth="1" />
      <path d="M10 22c4-8 16-8 20 0" fill="none" stroke="#e8c56b" strokeWidth="1.6" strokeLinecap="round" />
    ...
    <article className="cb-passport-light w-full max-w-[360px]">
      <div className="cb-passport-inner relative">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="rounded-pill bg-[#1ec8c0] px-2.5 py-1 text-[10px] font-bold text-[#0c3340]">FREE</span>
        </div>
        ...
        {skills.length ? (
          <div className="mt-2.5 flex flex-wrap gap-1">
            {skills.slice(0, 4).map((skill) => (
              <span key={skill} className="rounded-pill bg-[#1ec8c0] px-2 py-0.5 text-[9px] font-semibold text-[#0c3340]">
                {prettyText(skill)}
              </span>
            ))}
          </div>
