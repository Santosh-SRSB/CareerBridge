...
export default function ManualResumeEditor({ resumeId }) {
  const id = resumeId;
  const navigate = useRouter();
  const go = (path) => navigate.push(path);
  ...
  function goToStep(stepId) {
    ...
    if (stepId === "review") {
      window.requestAnimationFrame(() => {
        const panel = document.getElementById("ats-heading");
  ...
  function goNext() {
    ...
    if (currentIndex >= 0 && currentIndex < FORM_STEPS.length - 1) {
      setNavError("");
      const nextId = FORM_STEPS[currentIndex + 1].id;
      setEditorStep(nextId);
      if (nextId === "review") {
        window.requestAnimationFrame(() => {
          const panel = document.getElementById("ats-heading");
          if (panel) panel.scrollIntoView({ behavior: "smooth", block: "start" });
  ...
  if (error && !resume) {
    return (
      <div className="editor-error">
        ...
        <button className="btn" onClick={() => go("/resume/builder")}>Back to dashboard</button>
      </div>
    );
  }
  ...
  return (
    <div className="editor">
      <div className="editor-toolbar no-print">
        <button className="btn btn-ghost" onClick={() => go("/resume/builder")}>&larr; Dashboard</button>
        <input
                ...
                <div className="bullets">
                  ...
                  {job.bullets.map((b, j) => (
                    <div className="bullet-row" key={j}>
                      ...
                      <button className="btn btn-ghost btn-icon" type="button" onClick={() => removeBullet(i, j)}>Γ£ò</button>
                    </div>
                  ))}
                ...
                <div className="bullets">
                  ...
                  {(gap.activities || [""]).map((item, index) => (
                    <div className="bullet-row" key={`${gap.id}-act-${index}`}>
                      ...
                      />
                      <button className="btn btn-ghost btn-icon" type="button" onClick={() => removeCareerGapListItem(gap.id, "activities", index)}>Γ£ò</button>
                    </div>
                  ))}
                  ...
                  {(gap.skills || [""]).map((item, index) => (
                    <div className="bullet-row" key={`${gap.id}-skill-${index}`}>
                      <input
                        ...
                        placeholder="SQL"
                      />
                      <button className="btn btn-ghost btn-icon" type="button" onClick={() => removeCareerGapListItem(gap.id, "skills", index)}>Γ£ò</button>
                    </div>
                  ))}
                ...
                <div className="bullets">
                  ...
                  {(gap.certifications || [""]).map((item, index) => (
                    <div className="bullet-row" key={`${gap.id}-cert-${index}`}>
                      ...
                      />
                      <button className="btn btn-ghost btn-icon" type="button" onClick={() => removeCareerGapListItem(gap.id, "certifications", index)}>Γ£ò</button>
                    </div>
                  ))}
                ...
                <div className="bullets">
                  ...
                  {(gap.projects || [""]).map((item, index) => (
                    <div className="bullet-row" key={`${gap.id}-proj-${index}`}>
                      ...
                      />
                      <button className="btn btn-ghost btn-icon" type="button" onClick={() => removeCareerGapListItem(gap.id, "projects", index)}>Γ£ò</button>
                    </div>
                  ))}
              ...
              <div className="ready-prompt">
                ...
                <div className="ready-prompt-actions">
                  <button
                    ...
                    onClick={() => goToNextFeature(`/interviews${resume.data.targetRole ? `?role=${encodeURIComponent(String(resume.data.targetRole))}` : ""}`)}
                  >
                    {nextPathLoading.includes("/interviews") ? "Opening…" : "Start Mock Interview"}
                  </button>
                  <button
                    className="btn"
                    type="button"
                    disabled={Boolean(nextPathLoading) || saving}
                    onClick={() => goToNextFeature(`/resume/builder/${resume.id}/career-guidance`)}
                  >
