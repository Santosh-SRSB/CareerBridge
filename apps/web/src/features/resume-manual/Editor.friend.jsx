...
export default function Editor() {
  ...
  async function handleApplyRewrite() {
    ...
    try {
      ...
      setStatus("Rewrite applied in the editor ΓÇö click Save to keep it.");
      setError(err.message);
    }
  }
  ...
  if (!resume) return <p className="muted">LoadingΓÇª</p>;
            ...
            {(resume.data.education || []).map((ed, i) => (
              <div className="list-item education-card" key={ed.id}>
                <div className="education-card-head">
                  <h3>Education {i + 1}{ed.level ? ` ΓÇö ${ed.level}` : ""}</h3>
                </div>
                <div className="grid-2">
            ...
            <label>
              ...
              <div className="skill-tag-input">
                ...
                <input
                  ...
                  placeholder={(resume.data.skills || []).length === 0 ? "React, Node.js, SQLΓÇª" : ""}
                />
              </div>
            </label>
            <FormNav onPrevious={goPrevious} onNext={goNext} />
          ...
          <section className="form-section">
            ...
            <ul className="review-list">
              {FORM_STEPS.filter((step) => step.id !== "review").map((step) => (
                <li key={step.id}>
                  {sectionComplete(step.id, resume.data) ? "Γ£ô" : "Γùï"} {step.label}
                </li>
              ))}
            </ul>
            ...
            {resumeReadyForGuidance(resume.data) && (
              <div className="ready-prompt">
                ...
                <div className="ready-prompt-actions">
                  ...
                  >
                    {nextPathLoading.includes("/interviews/new") ? "OpeningΓÇª" : "Start Mock Interview"}
                  </button>
                  <button
                    className="btn"
                    type="button"
                  ...
                  >
                    {nextPathLoading.includes("career-guidance") ? "OpeningΓÇª" : "Skip for now"}
                  </button>
                </div>
              </div>
            )}
        ...
        <div className="editor-preview" ref={previewPaneRef}>
          ...
          {viewingRewrite && (
            <p className="ats-stale no-print preview-rewrite-note">
              Previewing rewritten resume ΓÇö the original is unchanged until you apply the rewrite.
            </p>
          )}
          {overfull && (
