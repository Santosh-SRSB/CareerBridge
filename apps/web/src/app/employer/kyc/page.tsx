...
export default function EmployerKycPage() {
  ...
  useEffect(() => {
    getEmployerMe()
      .then((profile) => {
        ...
        setTrademark(profile.companyName || '');
        setCin(profile.cin || '');
  ...
  async function onVerifyGst() {
    ...
    try {
      ...
      if (result.verified && result.status === 'ACTIVE') {
        ...
        setError('This GSTIN is not active. Enter an active GSTIN to continue.');
        setGstVerified(false);
        if (mark) setTrademark(mark);
      } else {
  ...
  async function onSubmit(event: FormEvent) {
    ...
    try {
      ...
      setGstVerified(true);
      const mark = (result.trademark || result.tradeName || trademark).trim();
      if (mark) setTrademark(mark);
