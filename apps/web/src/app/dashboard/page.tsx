...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([nextProfile, nextCompletion, recommended, resumes, interviews, nextApplications]) => {
        const displayName =
          [nextProfile.firstName, nextProfile.lastName].filter(Boolean).join(' ')
          || stored.firstName
          || 'there';
        setName(displayName);
    ...
    return (
      <div className="passport-page">
        <header className="passport-page-bar">
          <Link href="/" className="logo-mark shrink-0">
            <Image
        ...
        <main className="passport-page-stage">
          <PassportCard>
            <PassportStartChoices welcomeName={name} />
          </PassportCard>
        </main>
      </div>
    );
    ...
    <div className="cb-portal-page">
      ...
      <div className="cb-portal-wrap grid items-start gap-3 lg:grid-cols-[240px_minmax(0,1fr)] xl:grid-cols-[240px_minmax(0,1fr)_280px]">
        ...
        <div className="min-w-0 space-y-3">
          ...
          <section className="cb-hero-pop cb-dash-hero relative overflow-hidden p-4 sm:p-7">
            <div className="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
              <div className="min-w-0 flex-1">
            <p className="inline-flex items-center rounded-pill bg-gradient-to-r from-[#ca8a04] to-[#eab308] px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.16em] text-navy sm:text-[11px]">
              Free for Candidate
            </p>
            <h1 className="mt-3 text-[26px] font-extrabold tracking-tight text-white sm:mt-4 sm:text-[34px] sm:leading-tight">
              You&apos;re {completion.percentage}% ready.
              <span className="mt-1 block text-[#eab308]">Build your future.</span>
            </h1>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              <Link
                href="/jobs"
                className="inline-flex h-8 items-center rounded-full bg-gradient-to-r from-[#ca8a04] to-[#eab308] px-3.5 text-xs font-extrabold text-navy transition hover:brightness-110"
              >
                Explore Jobs
              </Link>
              <Link
                href="/interviews"
                className="gold-shimmer-btn inline-flex h-8 items-center rounded-full px-3.5 text-xs font-extrabold text-navy"
              >
            ...
            <div className="mt-5 grid grid-cols-3 gap-2">
              ...
              <div className="cb-ats-card shrink-0 self-center lg:self-start">
                <span className="cb-ai-ats-square" aria-hidden>
                  <strong>
                    AI
                    <span>ATS</span>
                  </strong>
              ...
              {improve.map((item) => (
                <li key={item.key}>
                  <Link href={item.href} className="cb-side-action">
                    ...
                    <span className="rounded-pill bg-gradient-to-r from-[#ca8a04] to-[#eab308] px-3 py-1 text-xs font-bold text-navy">Add</span>
                  </Link>
                </li>
              ))}
              {!hasResume ? (
                <li>
                  <Link href="/resume" className="cb-side-action">
                    ...
                    <span className="rounded-pill bg-gradient-to-r from-[#ca8a04] to-[#eab308] px-3 py-1 text-xs font-bold text-navy">Start</span>
                  </Link>
                </li>
              ) : null}
              {!improve.length && hasResume ? (
                <li className="rounded-xl bg-[#f6fbf7] px-3 py-3 text-sm text-muted">
