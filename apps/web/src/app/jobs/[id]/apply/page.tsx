...
import { submitApplication, fetchJobDetails } from '@/lib/candidate-marketplace-api';
...
export default function ApplyPage() {
  ...
  useEffect(() => {
    Promise.all([fetchJobDetails(params.id), listResumes().catch(() => [])])
  ...
  async function onSubmit(event: FormEvent) {
    ...
    setError('');
    try {
      // Resume is optional — backend attaches latest resume when available.
      const application = await submitApplication(params.id, resumeId || undefined);
  ...
  if (!job) {
    return (
      <CandidateAppShell activeTab="jobs">
        <p className="text-slate-500">Preparing your application...</p>
      </CandidateAppShell>
    );
  }
      ...
      <form onSubmit={onSubmit} className="cb-dash-card max-w-3xl space-y-5 p-4 sm:p-5">
        <div>
          ...
          <ul className="mt-3 space-y-1 text-sm text-primary">
            ...
            <li>{resumes.length ? '✓ Resume available' : '○ Resume optional (passport will be used)'}</li>
          </ul>
        </div>
