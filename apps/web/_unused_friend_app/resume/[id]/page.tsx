...
import { downloadResume, getResume, saveBase64File, updateResume } from '@/lib/api';
...
export default function ResumeEditorPage() {
  ...
  async function onDownload() {
    const file = await downloadResume(params.id);
    if (file.pdf) {
      saveBase64File(file.pdf, file.fileName, file.mimeType || 'application/pdf');
      return;
    }
    saveBase64File(file.html || '', file.fileName || 'career-passport.html', file.mimeType || 'text/html;charset=utf-8');
  }
  ...
  return (
    <CandidateShell>
      ...
      <section className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex w-full min-w-0 items-center gap-3 rounded-lg bg-white px-4 py-3 shadow-[0_16px_40px_rgba(12,51,64,0.1)] sm:w-auto">
      ...
      <div className="grid min-w-0 items-start gap-6 xl:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)]">
        <div className={showEditor ? 'block min-w-0' : 'hidden min-w-0 xl:block'}>
          <form onSubmit={onSave} className="cb-dash-card space-y-5 p-4 sm:p-5">
            ...
            {analysis ? (
              <div className="rounded-md bg-[#f7fbfb] p-4">
